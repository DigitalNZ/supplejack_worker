  ...
  describe 'lookup' do
    ...
    context 'match found' do
      ...
      it 'does not post an update if the source is reharvested' do
        ...
        expect(ApiUpdateWorker).not_to have_enqueued_sidekiq_job(
          '/harvester/concepts.json', {
            'concept' => {
              'internal_identifier' => 'http://www.mccahon.co.nz/',
              'source_id' => 'mccahon_co_nz',
              'sameAs' => 'http://www.mccahon.co.nz/',
              'match_status' => 'strong'
            }
          },
          nil
      ...
      it 'posts an update to the API with the sameAs and match_status fields' do
        worker.send(:lookup, attributes)
        expect(ApiUpdateWorker).to have_enqueued_sidekiq_job(
          '/harvester/concepts.json', {
            'concept' => {
              'internal_identifier' => 'http://www.mccahon.co.nz/',
              'source_id' => 'mccahon_co_nz',
